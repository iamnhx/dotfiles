...
defaults write NSGlobalDomain AppleLanguages -array "en_SG"

log_msg "Set time zone to Singapore"
sudo systemsetup -settimezone "Asia/Singapore" 2>/dev/null 1>&2
...
log_msg "Disable natural scrolling"
...
defaults write com.apple.dock wvous-br-corner -int 2 # Misson Control
defaults write com.apple.dock wvous-br-modifier -int 1048576
defaults write com.apple.dock wvous-tr-corner -int 11 # Launch Pad
...
log_msg "Enable grouping of windows by application"
defaults write com.apple.dock expose-group-apps -bool true

log_msg "Enable organizing of windows by application by mission control"
...
# If DockUtil installed, then use it to add useful ones
if ! hash dockutil 2>/dev/null; then
  # Download and install dockutil
  curl -LOJs https://github.com/kcrawford/dockutil/releases/download/3.0.2/dockutil-3.0.2.pkg  
  sudo installer -pkg dockutil-3.0.2.pkg -target / 
  rm dockutil-3.0.2.pkg
fi

apps_to_add_to_dock=(
  'Mail' 'Brave Browser' 'Discord' 'Telegram' 'WeChat' 'WhatsApp' 'BusyCal' 'Anybox' 'News Explorer' 
  'Fork' 'ForkLift' 'Notion' 'Numi' 'kitty' 'qBittorrent' 'Rclone Browser' 'Table Plus' 'Visual Studio Code'
  'System Settings'
)
IFS=""
...
for app in "${apps_to_add_to_dock[@]}"; do
  if [[ -d "/Applications/${app}.app" ]]; then
    dockutil --add "/Applications/${app}.app" > /dev/null 2>&1
  elif [[ -d "/System/Applications/${app}.app" ]]; then
    dockutil --add "/System/Applications/${app}.app" > /dev/null 2>&1
  fi
done
