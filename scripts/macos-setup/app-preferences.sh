...
if [[ ! $params == *"--skip-intro"* ]]; then
  ...
  echo -e "${PRIMARY_COLOR} MacOS App Preferencse Script${RESET_COLOR}"
...
theme=$(<config/macos/default.terminal)
plutil -replace 'Window Settings.Default' -xml "$theme" ~/Library/Preferences/com.apple.Terminal.plist
...
echo 'tell application "Terminal" to set current settings of first window to settings set "Default"' | osascript
